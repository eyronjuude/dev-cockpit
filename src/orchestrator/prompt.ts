...
export function buildInitialPrompt(input: BuildPromptInput): string {
  ...
  if (mode.editsCode) {
     ...
     * mode. `acceptEdits` permits file edits but denies every Bash and
     * PowerShell call, so telling the agent to verify its own work would be a
     * lie — and an expensive one, because it spends turns on calls that cannot
     * succeed. Observed on a real run: five denied read-only git commands.
     */
    const canRunCommands =
      effectivePermissionMode(project.agentPermissionMode, mode) === 'bypassPermissions';
