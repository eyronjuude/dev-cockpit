/**
 * Pattern-based secret masking, with no access to the environment.
 *
 * Split out of `redact.ts` so a browser bundle can run the same patterns. The
 * live log stream renders agent prose straight from an event payload, and only
 * the event *message* goes through the server-side pass — without this the
 * stream would be the one place in the UI where a leaked key survives.
 *
 * The environment scan stays in `redact.ts`: it reads `process.env`, which has
...
export const PATTERNS: ReadonlyArray<{ name: string; re: RegExp }> = [
  // Anthropic
...
/** Applies every pattern. Pure, and safe to run in either runtime. */
export function redactPatterns(input: string): RedactionResult {
...
export const redactPatternsText = (input: string): string => redactPatterns(input).text;
